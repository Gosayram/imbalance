...
async def ensure_vec_table(db: aiosqlite.Connection, dimension: int | None = None) -> bool:
	...
	await db.execute(
		f'CREATE VIRTUAL TABLE IF NOT EXISTS wiki_vec USING vec0(  embedding float[{dimension}])'
	)
	await db.commit()
	return True
...
async def search_by_embedding(
	...
	return [{'section_id': dict(r)['section_id'], 'distance': dict(r)['distance']} for r in rows]
